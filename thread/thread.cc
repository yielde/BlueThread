...
const pthread_t&
ThreadBase::get_thread_id() const
{
  return thread_id;
}

pid_t
ThreadBase::get_tid() const
{
  return tid;
}

void*
ThreadBase::_entry_func(void* arg)
{
...
int
ThreadBase::set_cpu_affinity(int cpuid)
{
  if (cpuid < 0 || cpuid >= CPU_SETSIZE) {
    std::cout << "invalid cpu id: " << cpuid << std::endl;
    return 0;
...
int
ThreadBase::try_create(int stack_size)
{
...
void
ThreadBase::create(const char* thread_name, int stack_size)
{
  ...
  if (r != 0) {
    char buf[1024];
    snprintf(
        buf, sizeof(buf), "failed to create thread(ThreadBase::try_create): %s",
        strerror(r));
...
ThreadBase::join(void** retval)
{
  ...
  if (r != 0) {
    char buf[1024];
    snprintf(
        buf, sizeof(buf), "failed to join thread(ThreadBase::join): %s",
        strerror(r));
    stderr_emergency(buf);
    ASSERT(r == 0);
  }
...
void
set_thread_name(std::thread& t, std::string& name)
{
  int r = pthread_setname_np(t.native_handle(), name.c_str());
  if (r != 0) {
    throw std::system_error(
        r, std::generic_category(), "failed to set thread name");
  }
}

std::string
get_thread_name(const std::thread& t)
{
  std::string name(16, '\0');
  // native_handle() is not const, but we only read the name, so use const_cast
  int r = pthread_getname_np(
      const_cast<std::thread&>(t).native_handle(), name.data(), name.size());
  if (r != 0) {
    throw std::system_error(
        r, std::generic_category(), "failed to get thread name");
  }
...
kill(std::thread& t, int signal)
{
  ...
  if (r != 0) {
    throw std::system_error(r, std::generic_category(), "failed to kill thread");
  }
}