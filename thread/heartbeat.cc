...
bool HeartbeatMap::_check(const heartbeat_handle *h, const char *who,
                          time now) const {
  bool healthy = true;
...
heartbeat_handle *HeartbeatMap::add_worker(const std::string &name,
                                           pthread_t thread_id) {
...
void HeartbeatMap::remove_worker(heartbeat_handle *h) {
  std::unique_lock locker{m_rwlock};
...
void HeartbeatMap::reset_timeout(
    heartbeat_handle *h, std::chrono::duration<uint64_t, std::nano> grace,
...
void HeartbeatMap::clear_timeout(heartbeat_handle *h) {
  auto now = clock::now();
...
int HeartbeatMap::get_unhealthy_workers() const { return m_unhealthy_workers; }

int HeartbeatMap::get_total_workers() const { return m_total_workers; }
